...
class Solution {
    ...
    private fun left(min: Int?, max: Int, node: TreeNode?): Boolean {
        ...
        return left(min, node.`val`, node.left) 
            && right(minOf(min ?: node.`val`, node.`val`), max, node.right)
    }
    ...
    private fun right(min: Int, max: Int?, node: TreeNode?): Boolean {
        ...
        return right(node.`val`, max, node.right)
            && left(node.`val`, maxOf(max ?: node.`val`, node.`val`), node.left)
    }
}
